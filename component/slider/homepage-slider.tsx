    ...
    <div ref={sliderRef} className="keen-slider pb-16 relative overflow-hidden">
      {mangas.map((manga, index) => {
        ...
        return (
          <div
            key={index}
            className="keen-slider__slide flex items-center justify-between px-40 md:px-40"
          >
            <div
              className={`w-2/3 pr-8 transition-all duration-700 ease-out ${currentSlide === index
                ? 'opacity-100 translate-x-0'
                : 'opacity-0 -translate-x-10'
                }`}
            >
              <p className="text-gray-300 mb-6 line-clamp-2 italic">Cập nhật lúc: {updateDay}</p>
              ...
              <p className="text-gray-300 mb-2 line-clamp-2"><span className='font-bold'>Thể loại: </span> {categoryNames}</p>
              <p className="text-gray-300 mb-6 line-clamp-2"><span className='font-bold'>Tình trạng: </span> {MangaStatus[manga.status as keyof typeof MangaStatus] || 'Không rõ'}</p>
