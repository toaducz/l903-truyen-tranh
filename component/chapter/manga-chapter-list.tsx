            ...
            <div className="space-y-2">
                {visibleChapters.map(item => (
                    <button
                        ...
                        onClick={() =>
                            router.push(
                                `/reader/${item.chapter_api_data}?slug=${slug}&chapter_name=${item.chapter_name ?? 'Không rõ'}`
                            )
                        }
                        className="w-full text-left p-3 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors cursor-pointer"
                    >
